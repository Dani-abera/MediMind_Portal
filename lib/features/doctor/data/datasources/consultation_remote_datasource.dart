...
class ConsultationRemoteDataSource {
  ...
  Future<VideoConsultationModel> join(String consultationId) async {
    final resp = await _client.dio
        .post('/api/v1/doctor/consultations/$consultationId/join');
    return VideoConsultationModel.fromJson(
        resp.data['data'] as Map<String, dynamic>);
  }
  ...
  Future<void> end(String consultationId) async {
    await _client.dio
        .post('/api/v1/doctor/consultations/$consultationId/end');
  }

  Future<VideoConsultationModel> getById(String id) async {
    final resp = await _client.dio.get('/api/v1/doctor/consultations/$id');
    return VideoConsultationModel.fromJson(
        resp.data['data'] as Map<String, dynamic>);
  }
  ...
  Future<List<VideoConsultationModel>> getActive() async {
    final resp =
        await _client.dio.get('/api/v1/doctor/consultations/active');
    final data = resp.data['data'] as List<dynamic>;
    return data
        .map((e) =>
            VideoConsultationModel.fromJson(e as Map<String, dynamic>))
        .toList();
  }

  Future<List<VideoConsultationModel>> getToday() async {
    final resp =
        await _client.dio.get('/api/v1/doctor/consultations/today');
    final data = resp.data['data'] as List<dynamic>;
    return data
        .map((e) =>
            VideoConsultationModel.fromJson(e as Map<String, dynamic>))
  ...
  Future<List<Map<String, dynamic>>> getChat(String consultationId) async {
    final resp = await _client.dio
        .get('/api/v1/doctor/consultations/$consultationId/chat');
    final data = resp.data['data'] as List<dynamic>;
