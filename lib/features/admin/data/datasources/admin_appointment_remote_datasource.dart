...
class AdminAppointmentRemoteDataSource {
  ...
  Future<List<AdminAppointmentModel>> getAppointments({
    ...
    final resp = await _client.dio.get('/api/v1/appointments', queryParameters: {
    ...
    return data.map((e) => AdminAppointmentModel.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<AdminAppointmentModel> getAppointmentDetail(String id) async {
    final resp = await _client.dio.get('/api/v1/appointments/$id');
    return AdminAppointmentModel.fromJson(resp.data['data'] as Map<String, dynamic>);
  }

  Future<void> approveAppointment(String id) async {
    await _client.dio.post('/api/v1/appointments/$id/approve');
  }

  Future<void> rejectAppointment(String id, {String? reason}) async {
    await _client.dio.post('/api/v1/appointments/$id/reject',
        data: {if (reason != null) 'reason': reason});
  }

  Future<void> cancelAppointment(String id, {String? reason}) async {
    await _client.dio.post('/api/v1/appointments/$id/cancel',
        data: {if (reason != null) 'reason': reason});
  }

  Future<void> bulkApprove(List<String> ids) async {
    await _client.dio.post('/api/v1/appointments/bulk-approve', data: {'ids': ids});
  }
}
